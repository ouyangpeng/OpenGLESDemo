...
void FBOLegLengthenSample::LoadImage(NativeImage *pImage) {
    LOGD("FBOLegLengthenSample::LoadImage pImage = %p", pImage->ppPlane[0])
    if (pImage) {
...
//	};
    ...
    if (m_FboProgramObj) {
        ...
        switch (m_StretchMode) {
            ...
            case HORIZONTAL_STRETCH_RIGHT_6_POINTS:
                ...
                break;
            default:
                LOGD("FBOLegLengthenSample::Init don't support this mode")
                return;
        }
        ...
        if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
            LOGD("FBOLegLengthenSample::Init glCheckFramebufferStatus status != GL_FRAMEBUFFER_COMPLETE")
        }
        glBindTexture(GL_TEXTURE_2D, GL_NONE);
    ...
    if (!CreateFrameBufferObj()) {
        LOGD("FBOLegLengthenSample::Init CreateFrameBufferObj fail")
        return;
    }
}
...
void FBOLegLengthenSample::Draw() {
    ...
    glDrawElements(GL_TRIANGLES, indicesNum, GL_UNSIGNED_SHORT, nullptr);
    GO_CHECK_GL_ERROR()
    glBindVertexArray(0);
...
bool FBOLegLengthenSample::CreateFrameBufferObj() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_RenderImage.width, m_RenderImage.height, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE, nullptr);
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        LOGD("FBOLegLengthenSample::CreateFrameBufferObj glCheckFramebufferStatus status != GL_FRAMEBUFFER_COMPLETE")
        return false;
    }
    glBindTexture(GL_TEXTURE_2D, GL_NONE);
    glBindFramebuffer(GL_FRAMEBUFFER, GL_NONE);
